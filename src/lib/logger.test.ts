...
Deno.test("Logger - デバッグモードのテスト", () => {
  ...
  try {
    ...
    assertEquals(consoleOutput.length, 0, "デバッグモードOFF時はメッセージを出力しない");

    // デバッグモードON時
    logger.setDebugMode(true);
    logger.debug("テストメッセージ");
    assertEquals(consoleOutput.length > 0, true, "デバッグモードON時はメッセージを出力する");

    // オブジェクトデータのデバッグ出力
    const testData = { key: "value" };
    logger.debug("テストデータ", testData);
    const output = consoleOutput.join("\\n");
    assertEquals(output.includes("value"), true, "オブジェクトデータが正しく出力される");
  } finally {
    teardownTest();
  }
});
...
Deno.test("Logger - デバッグモードのテスト - プリミティブ値", () => {
  ...
  try {
    ...
    assertEquals(consoleOutput.some(output => output.includes("文字列値")), true, "文字列データが正しく出力される");
    
    // 数値データ
    logger.debug("テストデータ", 123);
    assertEquals(consoleOutput.some(output => output.includes("123")), true, "数値データが正しく出力される");
    
    // undefined
    logger.debug("テストデータ");
    assertEquals(consoleOutput.some(output => !output.includes("undefined")), true, "undefinedの場合はデータを出力しない");
  } finally {
    teardownTest();
  }
});
...
Deno.test("Logger - エラーメッセージのテスト", () => {
  ...
  try {
    ...
    assertEquals(consoleOutput[1].includes("テストエラー"), true, "エラーオブジェクトのメッセージが含まれる");
  } finally {
    teardownTest();
  }
});
...
Deno.test("Logger - エラーメッセージのテスト - 非Error型のエラー", () => {
  ...
  try {
    ...
    assertEquals(consoleOutput[0].includes("エラーメッセージ: カスタムエラー"), true, "文字列エラーメッセージが含まれる");
    
    // オブジェクトエラー
    const customError = { message: "オブジェクトエラー" };
    logger.error("エラーメッセージ", customError);
    assertEquals(consoleOutput[1].includes("エラーメッセージ: [object Object]"), true, "オブジェクトがString変換されて出力される");
  } finally {
    teardownTest();
  }
});
